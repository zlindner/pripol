...
class OPP115:
    ...
    def __init__(self):
        ...
        self.stats['words_avg'] = self.stats['words_total'] / self.stats['total_segments']

        # apply stemming to each word in a segment TODO not sure why this results in more words???
        stemmer = PorterStemmer()
        self.consolidated['segment'] = self.consolidated['segment'].apply(
            lambda x: ' '.join([stemmer.stem(word) for word in word_tokenize(x)]))
        self.stats['words_stem'] = self.consolidated['segment'].apply(
    ...
    def encode(self):
        ...
        for data_practice in data_practices:
            def one_hot(x): return 1 if x.startswith(data_practice) else 0
    ...
    def display_statistics(self):
        ...
        print('Average number of words: %s' % self.stats['words_avg'])
