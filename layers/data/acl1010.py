...
def convert():
	...
	for filename in glob('acl1010/xml/*.xml'):
		...
		with open(filename, 'r', ) as f:
			...
			df.to_csv('acl1010/csv/' + filename[12:-4] + '.csv', sep=',', encoding='utf-8', index=False)
...
# loads each privacy policy csv into a single dataframe
def load():
	print('\n# loading acl1010')

	policies = []

	for filename in glob('acl1010/csv/*.csv'):
...
def train(acl1010):
	...
	model.save('acl1010/vec/acl1010.vec') # write model to disk

# displays the n most frequent words in the acl1010
def most_frequent(acl1010, n):
